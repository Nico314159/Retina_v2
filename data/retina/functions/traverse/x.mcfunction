...
execute if score $debug_messages __variable__ matches 1.. run tellraw @a ["",{"text":"Closest direction: ","bold":false,"color":"white"},{"text":"X","bold":true,"color":"aqua"}]
scoreboard players operation $distance_X __variable__ += $delta_X __variable__
scoreboard players operation $step_X __variable__ += $sign_X __variable__
scoreboard players operation $int_X __variable__ += $sign_X __variable__
